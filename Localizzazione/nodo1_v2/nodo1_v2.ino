...
void loop(){
  Apio.loop();
        ...
        if(millis()-preMillis1>3000){
          Apio.send("12:update:Persone:"+String(Apio.rssi[5])+"0"+String(Apio.rssi[6])+"0"+String(Apio.rssi[7])+"0"+String(Apio.rssi[8])+"0"+String(Apio.rssi[9])+"0"+String(Apio.rssi[10])+"-");
          ping++;  
          preMillis1 = millis();
          Serial.println("ping"); 
          Serial.println(ping);
