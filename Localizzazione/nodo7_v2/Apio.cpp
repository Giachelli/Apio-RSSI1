...
static bool receive(NWK_DataInd_t *ind) {
  if(Apio.isDongle)
  {
    ...
    } else if(abs(ind->rssi) < 72 && abs(ind->rssi) >= 54){
      digitalWrite(12,HIGH);
      digitalWrite(11,HIGH);
    } else if(abs(ind->rssi) < 54 && abs(ind->rssi) >= 36){
      digitalWrite(14,HIGH);
      digitalWrite(12,HIGH);
      digitalWrite(11,HIGH);
    } else if(abs(ind->rssi) < 36 && abs(ind->rssi) >= 18){
...
void appDataConf(NWK_DataReq_t *req)
{
  ...
  Apio.nwkDataReqB = 0;
  //Work Here
  Serial.println("Ack vale "+String(Apio.ack));
...
void ApioClass::loop() {
  ...
  if(this->isDongle){
    ...
    if(currentMillis - previousMillis > interval) {
      ...
      //digitalWrite(13,running_hearth);
      //digitalWrite(14,running_hearth);
      //digitalWrite(12,running_hearth);
      //digitalWrite(11,running_hearth);
...
//        display.display();
    ...
    if(!nwkDataReqB)
    {
      ...
      Serial.println("Voglio inviare perchè mex è vuoto");
