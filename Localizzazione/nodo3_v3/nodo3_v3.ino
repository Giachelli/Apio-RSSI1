...
int maxNode = 10;
void setup() {
  Apio.setup("Porta3", "1,0", addressNodo, 0x01);
...
void loop(){
  Apio.loop();
        ...
        if(millis()-preMillis1>3000){
          Apio.send("14:update:Persone:"+String(Apio.rssi[5])+"0"+String(Apio.rssi[6])+"0"+String(Apio.rssi[7])+"0"+String(Apio.rssi[8])+"0"+String(Apio.rssi[9])+"0"+String(Apio.rssi[10])+"-");
          ping++;
          preMillis1 = millis();
        }
